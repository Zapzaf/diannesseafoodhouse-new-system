        ...
        <div class="card p-4 shadow-sm">
            <div class="card-body p-0">
                ...
                <form action="{{ route('users.update', $user) }}" method="POST" enctype="multipart/form-data">
                    @csrf @method('PUT')
                    @php
                        // The ?v= cache-buster matters here: the URL itself never changes when
                        // a new photo is uploaded (it's keyed by user id, not filename), so
                        // without it the browser can keep showing the old cached image.
                        $existingProfilePhoto = $user->profile_photo_path
                            ? route('profile-photos.show', $user) . '?v=' . $user->updated_at?->timestamp
